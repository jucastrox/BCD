...
SELECT count(PrimeiroNome) AS Funcionario
FROM Funcionarios
...
SELECT count(codigo) AS funcionarios
FROM Funcionarios
...
SELECT count(codigo) AS funcionarios
FROM Funcionarios
WHERE SegundoNome OR ultimoNome = 'almeida' Or salario <3500;
...
SELECT SUM(salario) AS salario
FROM Funcionarios
WHERE funcao = 'analista' or salario >400;
